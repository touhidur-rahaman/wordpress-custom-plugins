 ...
 * 
 */
function xpdw_register_new_withdraw_method( $methods ){
    $methods['custom']    = [
        'title'     => __( 'Bkash Payment', 'dokan' ),
        'callback'  => 'xpdw_dokan_withdraw_method_bkash'
    ];

    return $methods;
}
add_filter( 'dokan_withdraw_methods', 'xpdw_register_new_withdraw_method', 99 );
 ...
 * 
 */
function xpdw_dokan_withdraw_method_bkash( $store_settings ){
    $value = isset( $store_settings['payment']['custom']['value'] ) ? esc_attr( $store_settings['payment']['custom']['value'] ) : ''; ?>

    <div class="dokan-form-group">
        <div class="dokan-w8">
            <div class="dokan-input-group">
                <span class="dokan-input-group-addon"><?php esc_html_e( 'Bkash Number', 'dokan-lite' ); ?></span>
                <input value="<?php echo esc_attr( $value ); ?>" name="settings[custom][value]" class="dokan-form-control value" placeholder="01......" type="text" required> 
            </div>
        </div>
    </div>
 ...
 * 
 */
function xpdw_custom_method_in_active_withdraw_method( $active_payment_methods, $vendor_id ) {
...
add_filter( 'dokan_get_seller_active_withdraw_methods', 'xpdw_custom_method_in_active_withdraw_method', 99, 2 );
 ...
 * 
 */
function xpdw_include_method_in_withdraw_method_section( $methods ){
    $methods[] = 'custom';
    return $methods;
}
add_filter( 'dokan_withdraw_withdrawable_payment_methods', 'xpdw_include_method_in_withdraw_method_section' );
